...
def load_config() -> Dict[str, Any]:
    ...
    return {
        ...
        'inverter_port': config.get('inverter_port') or int(environ.get('INVERTER_PORT', '12345')),
        'update_interval': config.get('update_interval') or int(environ.get('UPDATE_TIME', '30')),
        'mqtt_host': config.get('mqtt_host') or environ.get('MQTT_BROKER_IP', 'core-mosquitto'),
        'mqtt_port': config.get('mqtt_port') or int(environ.get('MQTT_BROKER_PORT', '1883')),
        ...
        'mqtt_topic_prefix': config.get('mqtt_topic_prefix') or environ.get('MQTT_INVERTER_TOPIC', 'homeassistant/sensor/solarmax'),
        ...
        'availability_topic': config.get('availability_topic', 'homeassistant/sensor/solarmax/availability'),
    }
...
logger.info(f"Starting Solarmax Agent with config: inverter={CONFIG['inverter_ip']}:{CONFIG['inverter_port']}, mqtt={CONFIG['mqtt_host']}:{CONFIG['mqtt_port']}")
...
UNITS_OF_MEASUREMENT = {
    "PAC": "W", "PDC": "W", "PD01": "W", "PD02": "W",
    "UL1": "V", "UL2": "V", "UL3": "V", "UD01": "V", "UD02": "V",
    "IL1": "A", "IL2": "A", "IL3": "A", "IDC": "A", "ID01": "A", "ID02": "A",
    "KDY": "Wh", "KMT": "kWh", "KYR": "kWh", "KT0": "kWh",
    "TKK": "°C", "KHR": "h", "CAC": "",
    "SAL": "", "SYS": ""
}
...
STATE_CLASSES = {
    "PAC": "measurement", "PDC": "measurement", "PD01": "measurement", "PD02": "measurement",
    "UL1": "measurement", "UL2": "measurement", "UL3": "measurement",
    "UD01": "measurement", "UD02": "measurement",
    "IL1": "measurement", "IL2": "measurement", "IL3": "measurement",
    "IDC": "measurement", "ID01": "measurement", "ID02": "measurement",
    "KDY": "total_increasing", "KMT": "total_increasing", 
    "KYR": "total_increasing", "KT0": "total_increasing",
    "TKK": "measurement", "KHR": "total_increasing", "CAC": "total_increasing"
}
...
class HomeAssistantMQTTPublisher:
    ...
    def _send_discovery_config(self, field: str, field_data: Dict[str, Any]):
        ...
        discovery_topic = f"{self.config['discovery_prefix']}/sensor/{self.config['device_id']}/{field.lower()}/config"
        ...
        if field in ["KDY", "KMT", "KYR", "KT0"]:
            ...
            if field == "KT0":  # Total energy - important for energy dashboard
                discovery_payload["last_reset_topic"] = f"{self.config['mqtt_topic_prefix']}/last_reset"
    ...
    def publish_data(self, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not self.discovery_sent and self.config.get('home_assistant_discovery', True):
...
class InverterConnection:
    ...
    def read_data(self, sock: socket.socket, request: str) -> str:
        ...
        try:
            ...
            while not data_received and (datetime.now() - start_time).total_seconds() < 2:
                buf = sock.recv(1024)
                if len(buf) > 0:
...
def generate_empty_data(field_map: Dict[str, str], 
                       last_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
